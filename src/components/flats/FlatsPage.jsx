...
const FlatsPage = () => {
  ...
  const filterBySearch = (e) => {
    ...
    const updated = (list || []).filter((item) =>
      item.flatNumber?.toLowerCase().includes(q.toLowerCase())
    );
    setFiltered(updated);
  };
      ...
      <div className="container-fluid ">
        ...
        <div className=" py-2">
          <div className="d-flex justify-content-between align-items-center mb-4">
            ...
            <div>
              <Link to='/dashboard/create-flat'> <button style={{ borderColor: "#F4B92D", color: '#F4B92D' }} className="btn rounded-circle"><i className="fas fa-plus "></i></button></Link>
            </div>
          </div>
          <div>
            {loading ? (
                      ...
                      <div className="d-flex align-items-center gap-3 flex-nowrap">
                        <div className="flex-grow-1">
                          <div className="d-flex align-items-center justify-content-between">
                            <h6 className="mb-1">Flat {e.flatNumber}</h6>
                          </div>
                        ...
                        <div className="text-end" style={{ minWidth: '110px' }}>
                          <Link to={`/dashboard/edit-flat/${e._id}`} className="btn btn-outline-dark btn-sm">Edit</Link>
                        </div>
                      </div>
                    </div>
                  </div>
