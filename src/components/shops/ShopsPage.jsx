...
const ShopsPage = () => {
  ...
  const applyFilters = useMemo(() => {
    ...
    return res.filter((item) =>
      String(item.shopNumber||'').toLowerCase().includes(String(q||'').toLowerCase())
    );
        ...
        <div className=" py-2">
          ...
          <div className="d-flex align-items-center gap-2 flex-wrap mb-2">
            ...
            <button className="btn btn-outline-dark ms-auto" onClick={()=>{
              ...
              window.open(`/pdf/shops-list${qs.toString() ? `?${qs.toString()}` : ''}`, '_blank');
            }}>Print</button>
          </div>
          <div>
            {loading ? (
                ...
                {(filtered || []).reverse().map((e) => (
                  <div key={e._id} className="col-12">
                    <div
                      ...
                      onClick={()=> window.open(`/dashboard/edit-shop/${e._id}`, '_blank')}
                    >
                      <div className="d-flex align-items-center gap-3 flex-nowrap">
                        <div className="flex-grow-1">
                          <div className="d-flex align-items-center justify-content-between">
                            <h6 className="mb-1">
                              Shop {e.shopNumber} | Owner: {e?.owner?.userName || '-'}
                            </h6>
                          </div>
                        ...
                        <div className="text-end" style={{ minWidth: '110px' }}>
                          <Link to={`/dashboard/edit-shop/${e._id}`} target="_blank" rel="noreferrer" className="btn btn-outline-dark btn-sm" onClick={(ev)=>ev.stopPropagation()}>Edit</Link>
                        </div>
                      </div>
                    </div>
                  </div>
