...
const EventsPage = () => {
  const { getEvents, getAdminMe } = useContext(AppContext);
  ...
  useEffect(() => {
    (async () => {
      ...
      const data = await getEvents({ from, to, q });
      setList(data || []);
      setLoading(false);
    })();
  }, [getEvents, startDate, endDate, q, getAdminMe]);
  ...
  return (
    <div className="my-2">
      <div className="container-fluid ">
        <h1 className="display-4" style={{ fontWeight: 900 }}>Events</h1>
        <div className=" py-2">
          <div className="d-flex justify-content-between align-items-center mb-2">
            <form onSubmit={filterBySearch}>
              <div className="d-flex align-items-center">
                <input value={q} onChange={(e) => setQ(e.target.value)} style={{ borderColor: "black", color: 'black', backgroundColor: "#ffffff" }} type="text" className="form-control" placeholder="Search Given From / Event" />
                <div className="px-2">
              ...
              <div className="row g-3">
                {(filtered || []).map((e) => (
                  <div key={e._id} className="col-12">
                    <div className="card border-0 shadow-sm p-2" style={{ cursor: 'pointer' }} onClick={()=> window.open(`/dashboard/edit-event/${e._id}`, '_blank')}>
                      <div className="d-flex align-items-center gap-3 flex-nowrap">
                        <div className="flex-grow-1">
                          <div className="d-flex align-items-center justify-content-between">
                            <h6 className="mb-1">{e.Event} | From: {e.GivenFrom}</h6>
                          </div>
                          <div className="small fw-bold">
                            Amount: {fmt(e?.amount)} | Received: {fmt(e?.paidAmount)} | Created: {d2(e?.dateOfCreation)}
                          </div>
                        </div>
                        <div className="text-end" style={{ minWidth: '110px' }}>
                          <Link to={`/dashboard/edit-event/${e._id}`} target="_blank" rel="noreferrer" className="btn btn-outline-dark btn-sm" onClick={(ev)=>ev.stopPropagation()}>Edit</Link>
                        </div>
                      </div>
                    </div>
                  </div>
