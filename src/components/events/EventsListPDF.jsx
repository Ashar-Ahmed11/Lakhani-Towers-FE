...
const EventsListPDF = () => {
  ...
  const { toPDF, targetRef } = usePDF({ filename: 'Events.pdf', resolution: Resolution.HIGH });
  ...
  useEffect(() => {
    (async()=>{
      setLoading(true);
      setList(await getEvents({ from: from?from.toISOString():undefined, to: to?to.toISOString():undefined }) || []);
      setLoading(false);
    })();
  }, [getEvents, from, to]);
