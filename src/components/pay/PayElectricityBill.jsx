...
const PayElectricityBill = () => {
  const { getElectricityBills, getElectricityBillById, payElectricityBill, getAdminMe } = useContext(AppContext);
  ...
  const onPay = async () => {
    try{
      ...
      history.push(`/pdf/pay-electricity-bill?${params.toString()}`);
